...
img = cv2.imread('..BmW.png', cv2.IMREAD_GRAYSCALE)
...
mun_labels, labelmap, stats, centers = output
colored = np.full((img.shape[0], img.shape[1], 3), 0, np.uint8)

for l in range(1, num_labels):
    if state[l][4] > 200:
        colored[labelmap == l] = (0, 255*l/num_labels, 255*(num_lebels-l)/num_labels)
        cv2.circle(colored, 
                   (int(centers[l][0], int(centers[l][1]))), 5, (255, 0, 0), cv2.cv2.FILLED)
