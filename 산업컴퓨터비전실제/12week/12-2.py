...
data = np.load('c:/study/12week/case1/stereo.npy').item()
Kl, Dl, Kr, Dr, R, T,img_size = data['Kl'], data['Dl'], data['Kr'], data['Dr'], \
                                data['R'], data['T'], data['img_size']

left_img = cv2.imread('c:/study/12week/case1/left14.png')
right_img = cv2.imread('c:/study/12week/case1/right14.png')

R1, R2, P1, P2, Q, validRoi1, validRoi2 = cv2.stereoRectify(Kl, Dl, Dr, img_size, R, T)

xmap1, ymap1 = cv2.initUnistortRectifyMap(Kl, Dl, R1, Kl, img_size, cv2.CV_32FC1)
xmap2, ymap2 = cv2.initUnistortRectifyMap(Kr, Dr, R2, Kr, img_size, cv2.CV_32FC1)
