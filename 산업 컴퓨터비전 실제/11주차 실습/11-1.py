...
pattern_size = (10,7)
samples = []
file_list = os.listdir('c:/study/11week/pinhole_calib')
img_file_list = [file for file in file_list if file.startswith('img')]

for filename in img_file_list:
    
    frame = cv2.imread(os.path.join('c:/study/11week/pinhole_calib', filename))
...
pattern_points = np.zero((np.prod(pattern_size), 3), np.float32)
pattern_point[:, :2] = np.indices(pattern_size).T.reshape(-1, 2)

images, corners = zip(*samples)

pattern_point = [pattern_points]*len(corners)
